...
client.on("ready", () => {
  ...
  client.user.setActivity(`${prefix}help | Excelsior Anouncement | Dev By ILYASS.`, { type: "WATCHING" });
});
  ...
  if (command == "say") {
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "**Tu na pas le Droit de faire cette commande.**"
      );
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@421711597141229569>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`${prefix}say\` :le bote repete ce que tu as dit
                        :heart: Dev By ILYASS.#6666 :heart: 
  `
...
`);
        ...
        .catch(() => {
          return message.channel
            .send(
              "**Vous devez autoriser à recevoir des messages en privé, afin de pouvoir vous envoyer des commandes **"
            )
            .then(() => {
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "giveaway")) {
    ...
    if (!message.guild.member(message.author).hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":heavy_multiplication_x:| **Vous devez avoir la fonction de modification sur le serveur**"
      );
    message.channel
      ...
      .then(msgg => {
        message.channel
          ...
          .then(collected => {
            ...
            if (!room)
              return message.channel.send(
                "**il n'a pas pu trouve le room | refet plus tard**"
              );
            room = collected.first().content;
            collected.first().delete();
            msgg.edit("**Notez le temps en minutes**").then(msg => {
              message.channel
                .awaitMessages(filter, {
                  max: 1, //HactorMC
                  time: 20000,
                ...
                .then(collected => {
                  if (isNaN(collected.first().content))
                    return message.channel.send(
                      ":heavy_multiplication_x:| **Vous devez définir une heure correcte.. ``Vous devez réécrire la commande``**"
                    );
                  duration = collected.first().content * 60000;
                  collected.first().delete();
                  msgg
                    .edit(
                      ":eight_pointed_black_star:| **Écrivez sur ce que vous voulez offrir**"
                    )
                    .then(msg => {
                      message.channel
                        .awaitMessages(filter, {
                        ...
                        .then(collected => {
                          ...
                          try {
                            ...
                            msgg.edit(
                              `:heavy_check_mark:| **le giveaway est préparé**`
                            );
                          } catch (e) {
                            msgg.edit(
                              `:heavy_multiplication_x:| **jai pas pus faire le giveaway car jai pas les permision**`
                            );
                            console.log(e);
                          }
                        });
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "settings")) {
    if (message.author.id !== message.guild.owner.user.id)
      return message.channel.send(
        "**:closed_lock_with_key: Pour des raisons de protection, les ordonnances de protection étaient limitées à Owners seulement**"
      );
    if (message.content.startsWith(prefix + "settings limitsban")) {
      if (!num) return message.channel.send("**:1234: | envoyer nombre ! **");
      if (isNaN(num)) return message.channel.send("**:1234: | nombre seulement ! **");
